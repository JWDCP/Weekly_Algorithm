...
·   메인 아이디어
    ...
    를 함께 관리해야 한다. 시작점에서부터 도착점이 아닌 특정 지점에 도달할 때까지 벽을 부수고 이동한 경로의 길이와 벽을 부수지 않고 이동한 경로의 길이가 같다면 굳이 부수지 않는 것이 낫다(벽을 제거한다 파괴한다 부순다 아주 표현이 뒤죽박죽 난리가 났다). 그 이유는 당연하게도 앞으로 갈 길에 '반드시 부숴야 하는' 벽이 존재할 수도 있기 때문이다.

    따라서 벽을 부수고 이동하는 거리와 벽을 부수지 않고 이동하는 거리가 같다면 당연히 벽을 부수지 않고 이동하는 방법을 택할 것이다. 또는 벽을 부수면 훨씬 거리가 단축되지만 벽을 부수지 않고 우회하여도 조금 더 오래 걸릴 뿐 도달을 할 수 있다면 그 경로도 유의미한 경로가 될 것이다.
...
·   알고리즘
    ...
    위 경우에서 breaking이 false에서 true가 될 수는 있지만 true에서 false가 될 순 없다. 즉, 벽을 부수지 않은 상태에서 부순 상태가 될 수는 있지만 부순 사실을 무를 수는 없는 것이다.
